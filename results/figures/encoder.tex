...
\begin{tikzpicture}[
    ...
    \node[block, text width=4.5cm, minimum height=2.8cm] (dense) at (7.5, -1.5) {};
    \node[font=\normalsize, anchor=north] at ($(dense.north) + (0,-0.2)$) {Dense (768→768)};
    % Input layer (bottom) - 13 nodes representing 768 (centered)
    \foreach [count=\i from 1] \x in {-1.8,-1.5,-1.2,-0.9,-0.6,-0.3,0.0,0.3,0.6,0.9,1.2,1.5,1.8} {
        \node[circle, draw, inner sep=1.5pt, minimum size=5pt, fill=gray!10] (din\i) at ($(dense.south) + (\x, 0.4)$) {};
    }
    % Output layer (top) - 13 nodes representing 768 (centered)
    \foreach [count=\i from 1] \x in {-1.8,-1.5,-1.2,-0.9,-0.6,-0.3,0.0,0.3,0.6,0.9,1.2,1.5,1.8} {
    ...
    \node[layer, text width=3.5cm, minimum height=1.5cm, above=0.9em of dense] (relu) {};
    ...
    \node[rectangle, draw, dashed, very thick, rounded corners=1mm, fill=white!10, 
          text width=3cm, minimum height=0.8cm, align=center, above=1.0em of relu] (dropout) 
          {Dropout (0.2)};
    ...
    \coordinate (linear_in) at ($(funnel_bl)!0.5!(funnel_br)$);
    
    \node[font=\normalsize, anchor=south] at ($(funnel_tl)!0.5!(funnel_tr) + (0, 0.15)$) {Linear (768→6)};
    
    % Input layer (bottom) - 12 nodes spread WIDE (representing 768)
    \foreach [count=\i from 1] \x in {-1.8,-1.5,-1.2,-0.9,-0.6,-0.3,0.0,0.3,0.6,0.9,1.2,1.5,1.8} {
    ...
    \coordinate (corner2) at (4.2, -3.6);
    \coordinate (corner3) at (7.5, -3.6);
    % Route the invisible path, but stop it slightly BELOW the Dense box border
    % so the visible connector can be trimmed to exactly meet the border
    % without drawing into the rounded corner.
    \path (cls.east) -| (corner1) |- (corner2) -- (corner3) |- ($(dense.south) + (0,-0.10)$);
    % Draw the visible connector without an arrow head and trim both ends so
    % the line cleanly meets the Dense box border (no overshoot).
    \draw[very thick, rounded corners=0.2cm, shorten >=2pt, shorten <=6pt, line cap=round]
        (cls.east) -| (corner1) |- (corner2) -- (corner3) |- ($(dense.south) + (0,-0.10)$);

    % Classification pipeline arrows
    \draw[arrow] (dense.north) -- (relu.south);
    \draw[arrow] (relu.north) -- (dropout.south);
    \draw[arrow] (dropout.north) -- (linear_in);
